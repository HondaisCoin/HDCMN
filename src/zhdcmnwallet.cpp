...
CzHDCMNWallet::CzHDCMNWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zhdcmn seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZHDCMNSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZHDCMNSeed_deprecated()) {
                    LogPrintf("%s: Updated zHDCMN seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zhdcmn seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zhdcmn wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzHDCMNWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZHDCMNCount(nCountLastUsed);
    else if (!walletdb.ReadZHDCMNCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzHDCMNWallet::Lock()
{
    seedMaster = 0;
}

void CzHDCMNWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzHDCMNWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZHDCMN(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzHDCMNWallet::LoadMintPoolFromDB()
{
...
void CzHDCMNWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzHDCMNWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzHDCMNWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zhdcmnTracker->HasPubcoinHash(pMint.first)) {
...
bool CzHDCMNWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZHDCMN(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zhdcmnTracker which also adds to database
    pwalletMain->zhdcmnTracker->Add(dMint, true);
...
void CzHDCMNWallet::SeedToZHDCMN(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzHDCMNWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzHDCMNWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZHDCMNCount(nCountLastUsed);
...
void CzHDCMNWallet::GenerateDeterministicZHDCMN(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzHDCMNWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzHDCMNWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
