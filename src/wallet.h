...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 HDCMN at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zHDCMN send
enum ZerocoinSpendStatus {
    ZHDCMN_SPEND_OKAY = 0,                            // No error
    ZHDCMN_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZHDCMN_WALLET_LOCKED = 2,                         // Wallet was locked
    ZHDCMN_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZHDCMN_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZHDCMN_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZHDCMN_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZHDCMN_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZHDCMN_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZHDCMN_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZHDCMN_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZHDCMN_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZHDCMN_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZHDCMN_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZHDCMN_SPENT_USED_ZHDCMN = 14,                      // Coin has already been spend
    ZHDCMN_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZHDCMN_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZHDCMNOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzhdcmnAuto) const;
    ...
    */
    ...
    CzHDCMNWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzHDCMNTracker> zhdcmnTracker;
    ...
    void setZWallet(CzHDCMNWallet* zwallet)
    {
        zwalletMain = zwallet;
        zhdcmnTracker = std::unique_ptr<CzHDCMNTracker>(new CzHDCMNTracker(strWalletFile));
    }

    CzHDCMNWallet* getZWallet() { return zwalletMain; }
    ...
    /** zHDCMN reset */
    boost::signals2::signal<void()> NotifyzHDCMNReset;
