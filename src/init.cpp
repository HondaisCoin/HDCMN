...
#include "config/hondaiscoinmn-config.h"
#endif
...
CzHDCMNWallet* zwalletMain = NULL;
...
void PrepareShutdown()
{
    ...
    RenameThread("hondaiscoinmn-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "hondaiscoinmn.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "hondaiscoinmnd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the HDCMN and zHDCMN money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 6802, 16802));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in HDCMN/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in HDCMN/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, hondaiscoinmn, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in HDCMN/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all HondaisCoinMN specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-hdcmnstake=<n>", strprintf(_("Enable or disable staking functionality for HDCMN inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zhdcmnstake=<n>", strprintf(_("Enable or disable staking functionality for zHDCMN inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:6802"));
    ...
    strUsage += HelpMessageOpt("-backupzhdcmn=<n>", strprintf(_("Enable automatic wallet backups triggered after each zHDCMN minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zhdcmnbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zHDCMN backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizehondaiscoinmnamount=<n>     " + strprintf(_("Keep N HDCMN anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 6801, 16801));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The HondaisCoinMN Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("hondaiscoinmn-loadblk");
...
/** Sanity checks
 *  Ensure that HondaisCoinMN is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. HondaisCoinMN Core is shutting down."));
...
#endif
    // Make sure only a single HondaisCoinMN process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. HondaisCoinMN Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("HondaisCoinMN version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //HondaisCoinMN: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //HondaisCoinMN specific: zerocoin and spork DB's
                ...
                // HondaisCoinMN: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZHDCMNMinted();
                        RecalculateZHDCMNSpent();
                    }
                    RecalculateHDCMNSupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // HondaisCoinMN: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of HondaisCoinMN Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart HondaisCoinMN Core to complete") << "\n";
        ...
        zwalletMain = new CzHDCMNWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zHDCMNWallet
        uiInterface.InitMessage(_("Syncing zHDCMN wallet..."));

        bool fEnableZPivBackups = GetBoolArg("-backupzhdcmn", true);
        pwalletMain->setZPivAutoBackups(fEnableZPivBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zhdcmnTracker->Init();
...
// XX42 Remove/refactor code below. Until then provide safe defaults
    nAnonymizeHondaisCoinMNAmount = 2;
...
//    }
//
//    nAnonymizeHondaisCoinMNAmount = GetArg("-anonymizehondaiscoinmnamount", 0);
//    if (nAnonymizeHondaisCoinMNAmount > 999999) nAnonymizeHondaisCoinMNAmount = 999999;
//    if (nAnonymizeHondaisCoinMNAmount < 2) nAnonymizeHondaisCoinMNAmount = 2;
    ...
    LogPrintf("Anonymize HondaisCoinMN Amount %d\n", nAnonymizeHondaisCoinMNAmount);
    ...
    /* Denominations
       ...
       For example:
       1HDCMN+1000 == (.1HDCMN+100)*10
       10HDCMN+10000 == (1HDCMN+1000)*10
    */
    obfuScationDenominations.push_back((200000 * COIN) + 10000000);
