...
// The zHDCMN block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zHDCMN is a hash of the serial
...
{
    //Create an output returning the zHDCMN that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZHDCMNOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zHDCMN output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zHDCMN", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZHDCMNOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zHDCMN output", __func__);
...
{
    CzHDCMNTracker* zhdcmnTracker = pwallet->zhdcmnTracker.get();
    CMintMeta meta;
    if (!zhdcmnTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zhdcmnTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a HDCMN stake is the outpoint
