...
CzHDCMNTracker::CzHDCMNTracker(std::string strWalletFile)
{
...
CzHDCMNTracker::~CzHDCMNTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzHDCMNTracker::Init()
{
...
bool CzHDCMNTracker::Archive(CMintMeta& meta)
{
...
bool CzHDCMNTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzHDCMNTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzHDCMNTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzHDCMNTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzHDCMNTracker::GetSerialHashes()
{
...
CAmount CzHDCMNTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzHDCMNTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzHDCMNTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzHDCMNTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzHDCMNTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzHDCMNTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzHDCMNTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzHDCMNTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzHDCMNTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzHDCMNTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzHDCMNTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzHDCMNTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzHDCMNTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzHDCMNTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzHDCMNTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzHDCMNTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzHDCMNTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzhdcmn from DB\n", __func__, listDeterministicDB.size());
    }
