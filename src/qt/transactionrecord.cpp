...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.IsZerocoinSpend() && (fZSpendFromMe || wallet->zhdcmnTracker->HasMintTx(hash))) {
            //zHDCMN stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZHDCMN;
        ...
        BOOST_FOREACH (const CTxOut& txout, wtx.vout) {
            ...
            if (mine) {
                ...
                if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address)) {
                    // Received by HondaisCoinMN Address
...
bool IsZHDCMNType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZHDCMN:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZHDCMN || type == TransactionRecord::MNReward) {
