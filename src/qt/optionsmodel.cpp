...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::HDCMN);
    ...
    if (!settings.contains("nAnonymizeHondaisCoinMNAmount"))
        settings.setValue("nAnonymizeHondaisCoinMNAmount", 1000);

    nAnonymizeHondaisCoinMNAmount = settings.value("nAnonymizeHondaisCoinMNAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeHondaisCoinMNAmount"))
        SoftSetArg("-anonymizehondaiscoinmnamount", settings.value("nAnonymizeHondaisCoinMNAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in hondaiscoinmn.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeHondaisCoinMNAmount:
            return QVariant(nAnonymizeHondaisCoinMNAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeHondaisCoinMNAmount:
            nAnonymizeHondaisCoinMNAmount = value.toInt();
            settings.setValue("nAnonymizeHondaisCoinMNAmount", nAnonymizeHondaisCoinMNAmount);
            emit anonymizeHondaisCoinMNAmountChanged(nAnonymizeHondaisCoinMNAmount);
            break;
        case CoinControlFeatures:
