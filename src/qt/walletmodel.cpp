...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered hondaiscoinmn address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 HDCMN.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal hondaiscoinmn:URI (hondaiscoinmn:XyZ...?message=example)
            {
...
static void NotifyzHDCMNReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzHDCMNReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzHDCMNReset.connect(boost::bind(NotifyzHDCMNReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzHDCMNReset.disconnect(boost::bind(NotifyzHDCMNReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zhdcmnTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
