...
public:
    ...
    bool WriteZHDCMNSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZHDCMNSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZHDCMNSeed_deprecated(uint256& seed);
    bool EraseZHDCMNSeed();
    bool EraseZHDCMNSeed_deprecated();

    bool WriteZHDCMNCount(const uint32_t& nCount);
    bool ReadZHDCMNCount(uint32_t& nCount);
