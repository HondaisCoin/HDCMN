...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zhdcmnTracker = unique_ptr<CzHDCMNTracker>(new CzHDCMNTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZHDCMN_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZHDCMN_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
